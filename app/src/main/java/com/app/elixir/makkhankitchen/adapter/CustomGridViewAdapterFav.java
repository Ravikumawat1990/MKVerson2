...
public class CustomGridViewAdapterFav extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        holder.imgAddToFav = (ImageView) grid.findViewById(R.id.imgAddToFav);
    ...
    public void filter(String charText) {
        ...
        pojoCategoryListings.clear();
        if (charText.length() == 0) {
            pojoCategoryListings.addAll(arraylist);
        } else {
            for (PojoCategoryListing contact : arraylist) {
                if (contact.getName().toLowerCase().contains(charText)) {
                    pojoCategoryListings.add(contact);
                }
            }
        }
